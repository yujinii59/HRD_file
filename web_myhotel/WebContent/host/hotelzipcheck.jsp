<%@page import="kr.stay.member.ZipcodeDto"%>
<%@page import="java.util.ArrayList"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<jsp:useBean id="memberMgr" class="kr.stay.member.MemberMgr" scope="page" />
...
function send(zip_narea1, zip_narea2, zip_narea3, zip_narea4, zip_narea5){
	var newaddr = zip_narea1 + " " + zip_narea2 + " " + zip_narea3 + " " + zip_narea4 + " " + zip_narea5;
	opener.document.hotFrm.hotel_address.value = newaddr;
	window.close();
}
</script>
</head>
<body>
<b>** 우편번호 찾기 **</b><br>
<form action="hotelzipcheck.jsp" name="zipForm" method="post">
<table>
  <tr>
  	<td>
...
%>
		<a href="javascript:send('<%=zip_narea1 %>','<%=zip_narea2 %>','<%=zip_narea3 %>','<%=zip_narea4 %>', '<%=zip_narea5 %>')">
		<%=zip_narea1 %> <%=zip_narea2 %> <%=zip_narea3 %> <%=zip_narea4 %> <%=zip_narea5 %> ( <%=zip_oarea1 %> <%=zip_oarea2 %> )
		</a>
		<br>

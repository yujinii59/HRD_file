...
public class MemberMgr {
	...
	public boolean checkId(String id) {
		...
		String sql = "select id from hotel_member where id = ?";
	...
	public boolean memberInsert(MemberBean bean) {
		...
		try {
			conn = ds.getConnection();
			String sql = "insert into hotel_member values(?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = conn.prepareStatement(sql);
			...
			pstmt.setString(8, bean.getNewaddress());
	...
	public boolean loginCheck(String id, String passwd) {
		...
		try {
			conn = ds.getConnection();
			String sql = "select id, passwd from hotel_member where id = ? and passwd = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
	...
	public MemberDto getMember(String id) {
		...
		try {
			conn = ds.getConnection();
			String sql = "select * from hotel_member where id=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			if(rs.next()) {
	...
	public boolean memberUpdate(MemberBean bean, String id) {
		...
		try {
			conn = ds.getConnection();
			String sql = "update hotel_member set passwd=?, birth=?, name=? , email=?, phone=?, zipcode=?, newaddress=?, address=?, detailaddress=?, job=? where id=?";
			pstmt = conn.prepareStatement(sql);
	...
	public boolean adminLoginCheck(String adminid, String adminpasswd) {
		...
		try {
			conn = ds.getConnection();
			String sql = "select * from siteadmin where adminid = ? and adminpasswd = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<MemberDto> getMemberAll(){
		...
		try {
			conn = ds.getConnection();
			String sql = "select * from hotel_member order by id asc";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public boolean deleteMem(String id) {
		...
		try {
			conn= ds.getConnection();
			String sql = "delete from hotel_member where id = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
			...
			System.out.println("deleteMem err : " + e);
		} finally { 
			try {
				if(rs != null) rs.close();
				if(pstmt != null) pstmt.close();
